...
namespace Sismeing.API.Controllers.Catalogo
{
    ...
    public class Tipo_EquipoController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Tipo_Equipo>>> GetAll()
        {
            return Ok(await _context.TiposEquipo.Where(x => x.Activo).ToListAsync());
        }

        [HttpGet("{id:int}")]
        public async Task<ActionResult<Tipo_Equipo>> GetById(int id)
        {
            var item = await _context.TiposEquipo.FirstOrDefaultAsync(x => x.Id == id);
            return item == null ? NotFound() : Ok(item);
        }

        [HttpPost]
        public async Task<ActionResult<Tipo_Equipo>> Create([FromBody] Tipo_Equipo item)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Tipo_Equipo item)
        {
        ...
        {
            var item = await _context.TiposEquipo.FindAsync(id);
