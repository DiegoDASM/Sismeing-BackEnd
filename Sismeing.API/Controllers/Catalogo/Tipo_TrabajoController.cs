...
namespace Sismeing.API.Controllers.Catalogo
{
    ...
    public class Tipo_TrabajoController : ControllerBase
    {
        ...
        public Tipo_TrabajoController(SupaBaseDBcontext context)
        {
            _context = context;
        }

        [HttpGet]
        public async Task<ActionResult<IEnumerable<Tipo_Trabajo>>> GetAll()
        {
            return Ok(await _context.TiposTrabajo.Where(x => x.Activo).ToListAsync());
        }

        [HttpGet("{id:int}")]
        public async Task<ActionResult<Tipo_Trabajo>> GetById(int id)
        {
            var item = await _context.TiposTrabajo.FirstOrDefaultAsync(x => x.Id == id);
            return item == null ? NotFound() : Ok(item);
        }

        [HttpPost]
        public async Task<ActionResult<Tipo_Trabajo>> Create([FromBody] Tipo_Trabajo item)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Tipo_Trabajo item)
        {
        ...
        {
            var item = await _context.TiposTrabajo.FindAsync(id);
