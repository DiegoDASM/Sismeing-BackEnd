...
namespace Sismeing.API.Controllers.Catalogo
{
    ...
    public class Tipo_MantenimientoController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Tipo_Mantenimiento>>> GetAll()
        {
            return Ok(await _context.TiposMantenimiento.Where(x => x.Activo).ToListAsync());
        }

        [HttpGet("{id:int}")]
        public async Task<ActionResult<Tipo_Mantenimiento>> GetById(int id)
        {
            var item = await _context.TiposMantenimiento.FirstOrDefaultAsync(x => x.Id == id);
            return item == null ? NotFound() : Ok(item);
        }

        [HttpPost]
        public async Task<ActionResult<Tipo_Mantenimiento>> Create([FromBody] Tipo_Mantenimiento item)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Tipo_Mantenimiento item)
        {
            if (id != item.Id) return BadRequest();

            var existente = await _context.TiposMantenimiento.FindAsync(id);
        ...
        {
            var item = await _context.TiposMantenimiento.FindAsync(id);
