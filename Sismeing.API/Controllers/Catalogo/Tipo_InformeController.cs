...
namespace Sismeing.API.Controllers.Catalogo
{
    ...
    public class Tipo_InformeController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Tipo_Informe>>> GetAll()
        {
            return Ok(await _context.TiposInforme.Where(x => x.Activo).ToListAsync());
        }

        [HttpGet("{id:int}")]
        public async Task<ActionResult<Tipo_Informe>> GetById(int id)
        {
            var item = await _context.TiposInforme.FirstOrDefaultAsync(x => x.Id == id);
            return item == null ? NotFound() : Ok(item);
        }

        [HttpPost]
        public async Task<ActionResult<Tipo_Informe>> Create([FromBody] Tipo_Informe item)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Tipo_Informe item)
        {
        ...
        {
            var item = await _context.TiposInforme.FindAsync(id);
