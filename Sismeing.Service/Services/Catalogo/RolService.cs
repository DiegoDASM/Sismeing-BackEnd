...
namespace Sismeing.Service.Services.Catalogo
{
    public class RolService : IRolService
    {
        ...
        public async Task<Rol> CreateAsync(Rol item, string usuarioRegistro)
        {
            ...
            var nombreNuevo = (item.NombreRol ?? string.Empty).Trim();
            if (nombreNuevo.Length == 0)
                throw new InvalidOperationException("El nombre es obligatorio.");
            if (await _context.Roles.AnyAsync(x => x.Activo && x.NombreRol.ToLower() == nombreNuevo.ToLower()))
                throw new InvalidOperationException($"Ya existe un rol con el nombre '{nombreNuevo}'.");
            item.NombreRol = nombreNuevo;
        ...
        public async Task<bool> UpdateAsync(int id, Rol item, string usuarioModificacion)
        {
            ...
            var nombreNuevo = (item.NombreRol ?? string.Empty).Trim();
            if (await _context.Roles.AnyAsync(x => x.Id != id && x.Activo && x.NombreRol.ToLower() == nombreNuevo.ToLower()))
                throw new InvalidOperationException($"Ya existe un rol con el nombre '{nombreNuevo}'.");
            item.NombreRol = nombreNuevo;
